...
def my_main(spark, my_dataset_dir):
    ...
    resultDF1 = inputDF.withColumn("station", pyspark.sql.functions.col("start_station_name")).groupBy(["station"]).agg({"start_station_name": "count"}).withColumnRenamed("count(start_station_name)", "num_departure_trips")
    resultDF2 = inputDF.withColumn("station2", pyspark.sql.functions.col("stop_station_name")).groupBy(["station2"]).agg({"stop_station_name": "count"}).withColumnRenamed("count(stop_station_name)", "num_arrival_trips")
    

    res = resultDF1.join(resultDF2, resultDF1["station"] == resultDF2["station2"], "full")

    res = res.withColumn("station", pyspark.sql.functions.coalesce(res["station"], res["station2"]))

    solutionDF = res.na.fill(value=0).select("station", "num_departure_trips", "num_arrival_trips").orderBy(res["station"])
